...
export type IrisRow = { x: [number, number, number, number]; y: 0 | 1 | 2 };

const RAW: Array<[number, number, number, number, 0 | 1 | 2]> = [
...
export function loadIris(): IrisRow[] {
  return RAW.map((r) => ({ x: [r[0], r[1], r[2], r[3]], y: r[4] }));
}